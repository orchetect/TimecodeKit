...
class TimecodeInterval_Tests: XCTestCase {
    ...
    func testInitA() throws {
        ...
        let intervalTC = try Timecode(.components(m: 1), at: ._24)
    ...
    func testInitC() {
        ...
        let intervalTC = Timecode(.components(m: -1), at: ._24, by: .allowingInvalid)
    ...
    func testIsNegative() {
        XCTAssertEqual(
            TimecodeInterval(Timecode(.zero, at: ._24))
                .isNegative,
            false
        )
        
        XCTAssertEqual(
            TimecodeInterval(Timecode(.zero, at: ._24), .minus)
                .isNegative,
            true
        )
    }
    ...
    func testTimecodeC() throws {
        ...
        let intervalTC = Timecode(.components(h: 26), at: ._24, by: .allowingInvalid)
        
        let interval = TimecodeInterval(intervalTC)
        
        XCTAssertEqual(
            interval.flattened(),
            try Timecode(.components(h: 02, m: 00, s: 00, f: 00), at: ._24)
        )
    }
    ...
    func testRealTimeValueA() throws {
        ...
        let intervalTC = try Timecode(.components(h: 1), at: ._24)
