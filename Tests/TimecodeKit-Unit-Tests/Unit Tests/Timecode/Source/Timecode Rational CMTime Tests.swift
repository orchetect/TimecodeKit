...
class Timecode_Rational_CMTime_Tests: XCTestCase {
    ...
    func testTimecode_cmTime() throws {
        ...
        try TimecodeFrameRate.allCases.forEach { fRate in
            let s = try Timecode.Components(m: 8, f: 20).toTimecode(at: fRate)
            let e = try Timecode.Components(m: 10, f: 5).toTimecode(at: fRate)
    ...
    func testTimecode_cmTime_SpotCheck() throws {
        let tc = try Timecode.Components(h: 00, m: 00, s: 13, f: 29).toTimecode(at: ._29_97_drop)
